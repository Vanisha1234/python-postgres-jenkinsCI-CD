...
var options, chart, linechartcustomerColors = getChartColorsArray("tryon-by-gender"),
    chartDonutBasicColors = (linechartcustomerColors && (options = {
        series: window.chartData ? window.chartData.tryon_by_gender_data.series : [0, 0, 0],
        labels: window.chartData ? window.chartData.tryon_by_gender_data.labels : ["Male", "Female", "Other"],
        chart: {
            height: 333,
            type: "donut"
        },
    ...
    }, (chart = new ApexCharts(document.querySelector("#tryon-by-gender"), options)).render()), getChartColorsArray("store-visits-source")),
    worldemapmarkers = (chartDonutBasicColors && (options = {
        series: window.chartData ? window.chartData.product_variants_by_category_data.series : [0, 0, 0],
        labels: window.chartData ? window.chartData.product_variants_by_category_data.labels : ["Clothes", "A", "J"],
        chart: {
            height: 333,
            type: "donut"
        },
